...
class Corr_AE(object):

    def __init__(self, input_len_img, input_len_txt):
        # data:
    ...
    def build_graph(self, latent_len):
        self.image_latent = image_encoder(self.image_ph, [2048, 1024, latent_len], self.keep_prob_ph,
                                          training=self.training_ph)
        self.image_tilde = image_decoder(self.image_latent, [1024, 2048, self.input_len_img], self.keep_prob_ph,
                                         training=self.training_ph)

        self.text_latent = text_encoder(self.text_ph, [1024, 1024, latent_len], self.keep_prob_ph,
                                        training=self.training_ph)
        self.text_tilde = text_decoder(self.text_latent, [1024, 1024, self.input_len_txt], self.keep_prob_ph,
                                       training=self.training_ph)
        ...
        self.latent_loss = tf.reduce_mean(tf.square(self.text_latent - self.image_latent))

        self._classify_vars()
        self._init_summary()

    @count_time
    def train(self, train_feed_dict: dict, test_feed_dict: dict):

        recon_loss = self.recon_image_loss + self.recon_text_loss
        latent_loss = self.latent_loss

        loss = 0.8 * recon_loss + 0.2 * latent_loss

        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            loss_opt = tf.train.AdamOptimizer(train_feed_dict['lr']).minimize(loss)
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(train_feed_dict['iteration']):
                ...
                _, image_loss_, text_loss_, latent_loss_ = sess.run(
                    [loss_opt, self.recon_image_loss, self.recon_text_loss, latent_loss], feed_dict=feed_dict)
                ...
                if i % 50 == 0:
                    print('At Iteration %d, recon image loss is %.5f, recon text loss is %.5f, latent loss %.5f' % (
                    i, image_loss_, text_loss_, latent_loss_))

                if i % 500 == 0 and i > 0:
                    image_query = sess.run(self.image_latent,
                                           feed_dict={self.image_ph: test_feed_dict['image_query'],
                                                      self.keep_prob_ph: 1.0,
                                                      self.training_ph: False})
                    text_query = sess.run(self.text_latent,
                                          feed_dict={self.text_ph: test_feed_dict['text_query'], self.keep_prob_ph: 1.0,
                                                     self.training_ph: False})

                    image_re = sess.run(self.image_latent,
                                        feed_dict={self.image_ph: test_feed_dict['image_re'],
                                                   self.keep_prob_ph: 1.0,
                                                   self.training_ph: False})

                    text_re = sess.run(self.text_latent,
    ...
    def eval(self, query, retrieval, similarity_matrix, query_modal, retrieval_modal, dis_metric, radius=None):
        ...
        if dis_metric == 'hash':
            ...
            precision, recall = precision_recall(radius, query_latent, retrieval_latent, similarity_matrix)
    ...
    def _init_summary(self):
        ...
        self.latent_loss_sum = tf.summary.scalar('latent_loss', self.latent_loss)
        ...
        self.loss_summaries.extend([self.latent_loss_sum,
                                    self.recon_image_loss_sum,
